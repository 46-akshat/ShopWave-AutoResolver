...
#             # The 20% Hackathon Trap
#             if random.random() < 0.20:
#                 raise TimeoutError("Legacy Billing API failed to respond.")
            
#             # If it succeeds (doesn't raise an error), break out of the loop and continue!
#             break 
            
#         except TimeoutError:
#             if attempt == 2: # If it failed 3 times, give up and tell the AI to escalate.
#                 return {"eligible": False, "reason": "SYSTEM ERROR: API down after 3 retries. You MUST escalate."}
            
#             print(f"⚠️ API failed. Retrying... (Attempt {attempt + 1})")
...
@tool
async def get_order(order_id: str) -> Dict[str, Any]:
    """Mock tool: Fetches order details."""
    await asyncio.sleep(random.uniform(0.2, 0.4))
    if order_id not in ORDERS_DB:
        return {"status": "error", "message": f"Order {order_id} not found."}
...
@tool
async def check_refund_eligibility(order_id: str, customer_email: str = None) -> Dict[str, Any]:
    """Mock tool: Verifies system-level eligibility and flags fraud."""
    for attempt in range(3):
        try:
            await asyncio.sleep(1.0)
            if random.random() < 0.20:
            ...
            if attempt == 2:
                return {"eligible": False, "reason": "SYSTEM ERROR: API down. You MUST escalate."}
    ...
    if customer_email and customer_email in CUSTOMERS_DB:
        ...
        # Only escalate if there's a serious pattern (e.g., 4+ refunds)
        if "4 refund requests" in hist:
            return {
                "eligible": True, 
                "warning": "HIGH REFUND FREQUENCY: Potential fraud pattern detected.",
                "escalate_recommended": True 
            }
        
        # For general history (like Alice), we warn but DON'T force escalation
        if "refund" in hist:
            return {
                "eligible": True, 
                "warning": "Customer has a history of prior refunds. Verify condition.",
...
@tool
async def issue_refund(order_id: str, amount: float) -> str:
    """Mock tool: IRREVERSIBLE ACTION with $200 safety limit."""
    await asyncio.sleep(1.0)
    if amount > 200:
        return f"ERROR: Refund of ${amount} exceeds $200 limit. Process Blocked. Escalate."
...
@tool
async def escalate(ticket_id: str, reason: str, attempts_made: str, suggested_resolution: str, priority: str = "medium") -> str:
    """Mock tool: Structured hand-off to human support."""
    await asyncio.sleep(0.3)
    return (
        f"ESCALATION SUCCESSFUL for Ticket {ticket_id}.\n"
